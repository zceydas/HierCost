...
SRsumm <- summarySEwithin(data=branchsrlong, measurevar='selectionrate', withinvars='condition', idvar='ID')
...
p_ac <- ggplot(data = SRsumm,aes(y=selectionrate,x=condition)) +
  ...
  geom_point(data=branchsrlong, aes(y=selectionrate, x=condition)) + 
...
branchRTssumm <- summarySEwithin(data=branchRTslong, measurevar='RT', withinvars=c('condition', 'Phase'), idvar='ID', na.rm=T)

branchRTg <- ggplot(branchRTslong, aes(x=condition, y=RT, shape=Phase, fill=Phase)) +
...
branchAccg <- ggplot(branchAcclong, aes(x=condition, y=Acc, shape=Phase, fill=Phase)) +
  geom_boxplot(outlier.shape=NA) + 
  geom_point(position=position_jitterdodge(jitter.width=.09, dodge.width=.75)) + 
