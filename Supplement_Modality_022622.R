...
branch_supp_2$Abstraction <- 1+branch_supp_2$Abstraction

#Branch: iterative search
fitwell1 <- glmulti(selectionrate ~ 1 + Abstraction + SelectionRT + SelectionErr, data=branch_supp, 
                    level = 2, fitfunc = lmer.glmulti,confsetsize = 1024, random = "+(1+Abstraction + SelectionRT + SelectionErr ||Subject)", method = "h") 
...
comb_supp_4$Abstraction <- 1+comb_supp_4$Abstraction
...
#Branch: iterative search
fitwell2 <- glmulti(selectionrate ~ 1 + Abstraction + SelectionRT + SelectionErr, data=comb_supp_2, 
                    level = 2, fitfunc = lmer.glmulti,confsetsize = 1024, random = "+(1+Abstraction + SelectionRT + SelectionErr ||Subject)", method = "h") 
...
term1_c <- lmer(selectionrate ~ 1 + Abstraction + (Abstraction ||Subject), data = na.omit(comb_supp_2), REML=F)
term12_c <- lmer(selectionrate ~ 1 + SelectionErr + Abstraction + (Abstraction + SelectionErr ||Subject), data = na.omit(comb_supp_2), REML=F)
